...
import { GetReviewsByReviewerId } from "../../services/reviewsService";
import RemoveReview from "../Modals/DeleteReviewModal";
...
export default function UserReviews() {
    ...
    useEffect(() => {
        const fetchUserReviews = async () => {
            ...
            try {
                const userReviewsResponse = await GetReviewsByReviewerId(userId);
                setReviews(userReviewsResponse.data);
            } catch {
    ...
    const handleToggleComment = (projectId: string) => {
        setExpandedComment((prev) => (prev === projectId ? null : projectId));
    };
    ...
    if (loading) {
        return (
            <p className="text-center text-gray-200">Loading your reviews...</p>
        );
    }
                        ...
                        <table className="min-w-full bg-gray-800 border border-gray-700">
                            <thead>
                                <tr className="bg-gray-900 text-gray-300">
                                    <th className="py-3 px-2 text-left">Reviews</th>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div>
                                                    <h3
                                                        ...
                                                        onClick={() =>
                                                            navigate(
                                                                `/Session/${encodeURIComponent(review.sessionId!)}`
                                                            )
                                                        }
                                                    >
                                                        {review.sessionId}
